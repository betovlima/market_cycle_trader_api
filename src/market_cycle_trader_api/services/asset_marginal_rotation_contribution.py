...
class MarginalCandidateEvaluation:
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "marginal_forward_net_log_return_sum": float(self.marginal_forward_net_log_return_sum),
            "marginal_forward_net_log_return_mean": float(self.marginal_forward_net_log_return_mean),
            "marginal_forward_net_log_return_median": float(self.marginal_forward_net_log_return_median),
...
def _normalize_predictions(predictions: pd.DataFrame) -> pd.DataFrame:
    required = {"fold", "timestamp", "symbol", "predicted_utility", "realized_utility", "forward_net_log_return"}
    missing = sorted(required.difference(predictions.columns))
    if missing:
        raise RuntimeError("Leadership predictions are missing required columns: " + ", ".join(missing))
    frame = predictions[list(required)].copy()
    frame["symbol"] = frame["symbol"].astype(str).str.strip().str.upper()
    frame["timestamp"] = pd.to_datetime(frame["timestamp"], utc=True)
    frame["fold"] = pd.to_numeric(frame["fold"], errors="coerce").astype("Int64")
    for column in ("predicted_utility", "realized_utility", "forward_net_log_return"):
        frame[column] = pd.to_numeric(frame[column], errors="coerce")
    frame = frame.dropna(subset=["fold", "timestamp", "symbol", "predicted_utility"])
    frame["fold"] = frame["fold"].astype(int)
    return frame.sort_values(["fold", "timestamp", "symbol"]).drop_duplicates(
        subset=["fold", "timestamp", "symbol"], keep="last"
    ).reset_index(drop=True)


def _metric_pivots(frame: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    index = ["fold", "timestamp"]
    predicted = frame.pivot(index=index, columns="symbol", values="predicted_utility").sort_index()
    realized_utility = frame.pivot(index=index, columns="symbol", values="realized_utility").reindex(predicted.index)
    forward_net = frame.pivot(index=index, columns="symbol", values="forward_net_log_return").reindex(predicted.index)
    return predicted, realized_utility, forward_net


def _leader_state(predicted: pd.DataFrame, realized_utility: pd.DataFrame, forward_net: pd.DataFrame, selected_assets: list[str]) -> pd.DataFrame:
...
def _candidate_events(*, step: int, candidate: str, selected_assets: list[str], predicted: pd.DataFrame, realized_utility: pd.DataFrame, forward_net: pd.DataFrame) -> tuple[MarginalCandidateEvaluation, pd.DataFrame]:
    leader = _leader_state(predicted, realized_utility, forward_net, selected_assets)
    if candidate not in predicted.columns:
        return MarginalCandidateEvaluation(step, candidate, len(selected_assets), 0, 0, float("-inf"), float("nan"), float("nan"), float("nan"), float("nan"), float("nan"), float("nan")), pd.DataFrame()
    ...
    if events.empty:
        return MarginalCandidateEvaluation(step, candidate, len(selected_assets), 0, 0, 0.0, float("nan"), float("nan"), float("nan"), float("nan"), float("nan"), float("nan")), pd.DataFrame()
    ...
    events["predicted_score_gap"] = events["candidate_predicted_utility"] - events["leader_predicted_utility"]
    events["marginal_realized_utility"] = events["candidate_realized_utility"] - events["leader_realized_utility"]
    events["marginal_forward_net_log_return"] = events["candidate_forward_net_log_return"] - events["leader_forward_net_log_return"]
    events = events.reset_index()
    events = events.loc[np.isfinite(pd.to_numeric(events["marginal_forward_net_log_return"], errors="coerce"))].copy()
    if events.empty:
        return MarginalCandidateEvaluation(step, candidate, len(selected_assets), 0, 0, 0.0, float("nan"), float("nan"), float("nan"), float("nan"), float("nan"), float("nan")), events
    ...
    evaluation = MarginalCandidateEvaluation(
        ...
        marginal_realized_utility_sum=float(fu.sum()) if not fu.empty else float("nan"),
        marginal_realized_utility_mean=float(fu.mean()) if not fu.empty else float("nan"),
        predicted_score_gap_mean=float(fg.mean()) if not fg.empty else float("nan"),
    )
    return evaluation, events


def greedy_marginal_rotation_selection(*, predictions: pd.DataFrame, baseline_assets: list[str], candidate_assets: list[str]) -> MarginalRotationSelectionResult:
    frame = _normalize_predictions(predictions)
    predicted, realized_utility, forward_net = _metric_pivots(frame)
    baseline = list(dict.fromkeys(str(s).strip().upper() for s in baseline_assets if str(s).strip()))
    if len(baseline) < 2:
        raise RuntimeError("Marginal rotation selection requires at least two baseline assets.")
    missing = [s for s in baseline if s not in predicted.columns]
    if missing:
        raise RuntimeError("Baseline assets are missing from leadership predictions: " + ", ".join(missing))
    baseline_set = set(baseline)
    remaining = sorted({str(s).strip().upper() for s in candidate_assets if str(s).strip() and str(s).strip().upper() not in baseline_set})
    ...
    while remaining:
        results = [
            _candidate_events(step=step, candidate=c, selected_assets=selected_assets, predicted=predicted, realized_utility=realized_utility, forward_net=forward_net)
            for c in remaining
        ]
        evaluations.extend(e.as_dict() for e, _ in results)
        best_evaluation, best_events = sorted(
            results,
            key=lambda item: (-float(item[0].marginal_forward_net_log_return_sum), item[0].candidate),
        )[0]
        ...
        steps.append({**best_evaluation.as_dict(), "selected": True, "resulting_universe_size": len(selected_assets)})
    ...
    return MarginalRotationSelectionResult(
        ...
        selected_events=pd.concat(selected_events, ignore_index=True) if selected_events else pd.DataFrame(),
    )
