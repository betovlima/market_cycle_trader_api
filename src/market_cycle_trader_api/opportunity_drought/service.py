...
from ..infrastructure.persistence.mongo_repository import TEMPORAL_OPPORTUNITY_DROUGHT_RESEARCH_COLLECTION, bson_value
...
def _ensure_indexes(db: Any) -> None:
    collection = db[TEMPORAL_OPPORTUNITY_DROUGHT_RESEARCH_COLLECTION]
    collection.create_index(
        [("run_id", ASCENDING), ("processing_id", ASCENDING), ("period_start", ASCENDING), ("period_end", ASCENDING), ("created_at", DESCENDING)],
        name="ix_opportunity_drought_scope_created",
    )
    collection.create_index([("id", ASCENDING)], unique=True, name="uq_opportunity_drought_id")
...
def get_persisted(
    ...
    *,
    ...
    row = db[TEMPORAL_OPPORTUNITY_DROUGHT_RESEARCH_COLLECTION].find_one(query, {"_id": 0}, sort=[("created_at", DESCENDING)])
    return bson_value(row) if row is not None else None


def _save(db: Any, document: dict[str, Any]) -> dict[str, Any]:
    _ensure_indexes(db)
    db[TEMPORAL_OPPORTUNITY_DROUGHT_RESEARCH_COLLECTION].insert_one(dict(document))
    return document
...
def build_and_persist(
    ...
    *,
    ...
    if existing and str(existing.get("status") or "").lower() == "completed":
        return existing
    leadership = get_leadership_regime(
        db,
        run_id,
    ...
    if not leadership or str(leadership.get("status") or "").lower() != "completed":
        raise ValueError("Opportunity Drought Research requires completed Leadership Regime diagnostics.")
...
def public_summary(document: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    payload.pop("oos_sessions", None)
    return bson_value(payload)


def delete_run_results(db: Any, run_id: str) -> int:
    return int(db[TEMPORAL_OPPORTUNITY_DROUGHT_RESEARCH_COLLECTION].delete_many({"run_id": str(run_id)}).deleted_count or 0)
