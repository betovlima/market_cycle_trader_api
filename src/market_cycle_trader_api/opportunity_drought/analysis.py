...
def _cohort_summary(monthly: list[dict[str, Any]], outcome: str | set[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "average_drought_probability": _mean([row.get("oos_drought_probability") for row in rows]),
        "average_no_good_opportunity_share": _mean([row.get("no_good_opportunity_share") for row in rows]),
        "features": {
...
def _readiness(folds: list[dict[str, Any]], shadow: dict[str, Any]) -> dict[str, Any]:
    aucs = [
        _number(((row.get("monthly_metrics") or {}).get("auc")))
        for row in folds
    ]
    aucs = [value for value in aucs if value is not None]
    flagged_5 = _number(((shadow.get("flagged") or {}).get("average_forward_return_5")))
    unflagged_5 = _number(((shadow.get("unflagged") or {}).get("average_forward_return_5")))
    if not aucs:
        status = "insufficient_data"
    elif min(aucs) >= 0.60 and flagged_5 is not None and unflagged_5 is not None and flagged_5 < unflagged_5:
        status = "consistent_research_signal"
    elif sum(aucs) / len(aucs) >= 0.60 and min(aucs) >= 0.55:
    ...
    return {
        "status": status,
        "policy_ready": False,
        "requires_financial_replay": True,
        "reason": "Opportunity Drought remains research-only until the signal is stable across OOS folds and a cost-aware CASH replay improves capital without damaging positive months.",
    }
...
def build_analysis(
    ...
    *,
    ...
    if not session_source or not official_map:
        raise ValueError("Opportunity Drought Research requires Leadership Regime sessions and official Strategy monthly returns.")
    ...
    for item in session_source:
        ...
        for feature in FEATURES:
            row[feature] = _number(features.get(feature))
        rows.append(row)
    frame = pd.DataFrame(rows)
    if frame.empty:
        raise ValueError("Opportunity Drought Research has no sessions bound to official monthly returns.")
    ...
    for test_fold in folds[1:]:
        ...
        output = test.copy()
        output["oos_drought_probability"] = probability
        output["drought_signal"] = probability >= threshold
        ...
        monthly_test = output.groupby("month", as_index=False).agg(
            ...
            oos_drought_probability=("oos_drought_probability", "mean"),
            drought_signal_share=("drought_signal", "mean"),
        )
        fold_reports.append({
            ...
            "monthly_metrics": _metrics(
                ...
                monthly_test["oos_drought_probability"].to_numpy(dtype=float),
                threshold,
            ),
        })
    ...
    if not oos.empty:
        for month, group in oos.groupby("month"):
            contributions = []
            for feature in FEATURES:
                value = _mean(group[f"contribution__{feature}"].tolist())
                contributions.append({"feature": feature, "contribution": value})
            contributions.sort(key=lambda row: abs(float(row.get("contribution") or 0.0)), reverse=True)
            oos_by_month[str(month)] = {
                "oos_drought_probability": _mean(group["oos_drought_probability"].tolist()),
                "drought_signal_share": _mean(group["drought_signal"].astype(float).tolist()),
    ...
    for month in sorted(by_month):
        ...
        state_share = sum(1 for row in month_rows if row.get("state") == "no_good_opportunity") / len(month_rows)
        monthly.append({
            "month": month,
            ...
            "no_good_opportunity_share": float(state_share),
            "oos_drought_probability": (oos_by_month.get(month) or {}).get("oos_drought_probability"),
            "drought_signal_share": (oos_by_month.get(month) or {}).get("drought_signal_share"),
    ...
    if not oos.empty:
        for _, item in oos.iterrows():
            session = {
                ...
                "oos_drought_probability": _number(item.get("oos_drought_probability")),
                "drought_signal": bool(item.get("drought_signal")),
                "realized_forward_return_1": _number(item.get("realized_forward_return_1")),
                "realized_forward_return_5": _number(item.get("realized_forward_return_5")),
                "realized_forward_return_10": _number(item.get("realized_forward_return_10")),
    ...
    if not oos.empty:
        for feature in FEATURES:
            ...
            focus = oos[oos["month"] == FOCUS_MONTH]
            feature_importance.append({
                ...
                "focus_month_contribution": _mean(focus[column].tolist()),
            })
        feature_importance.sort(key=lambda row: float(row.get("mean_abs_contribution") or 0.0), reverse=True)

    shadow: dict[str, Any] = {"flagged_sessions": 0, "flagged_share": 0.0, "flagged": {}, "unflagged": {}}
    if not oos.empty:
        flagged = oos[oos["drought_signal"]]
        unflagged = oos[~oos["drought_signal"]]
        shadow = {
            "flagged_sessions": int(len(flagged)),
            "flagged_share": float(len(flagged) / len(oos)) if len(oos) else 0.0,
            "flagged": {
                "average_forward_return_1": _mean(flagged["realized_forward_return_1"].tolist()),
                "average_forward_return_5": _mean(flagged["realized_forward_return_5"].tolist()),
                "average_forward_return_10": _mean(flagged["realized_forward_return_10"].tolist()),
            },
            "unflagged": {
                "average_forward_return_1": _mean(unflagged["realized_forward_return_1"].tolist()),
                "average_forward_return_5": _mean(unflagged["realized_forward_return_5"].tolist()),
                "average_forward_return_10": _mean(unflagged["realized_forward_return_10"].tolist()),
            },
        }
    ...
    if focus_row is not None:
        focus_detail = dict(focus_row)
        focus_detail["feature_percentiles_vs_positive_months"] = {
            feature: _percentile(
    ...
    if not oos.empty:
        aggregated = oos.groupby("month", as_index=False).agg(
            target_negative_month=("target_negative_month", "first"),
            oos_drought_probability=("oos_drought_probability", "mean"),
            drought_signal=("drought_signal", "mean"),
        )
        aggregate_month_metrics = _metrics(
            aggregated["target_negative_month"].to_numpy(dtype=int),
            aggregated["oos_drought_probability"].to_numpy(dtype=float),
            0.5,
        )
    ...
    result = {
        ...
        "method": {
            "name": "opportunity_drought_monthly_outcome_learning",
            "model": "lightgbm",
            "baseline_model_comparison": False,
            ...
            "policy_effect": "none",
            "purpose": "explain whether opportunity drought is a stable OOS failure family before any CASH intervention",
        },
        "summary": {
            "months": len(monthly),
        ...
        "focus_month": focus_detail,
        "monthly": monthly,
        "oos_sessions": oos_sessions,
    }
    result["readiness"] = _readiness(fold_reports, shadow)
    return result
