...
def _selected_asset_features(db: Any, run_id: str, selected_by_day: dict[str, str]) -> dict[str, dict[str, Any]]:
    ...
    for document in cursor:
        day = _date_key(document.get("timestamp"))
        expected = selected_by_day.get(day)
        if not expected:
            continue
        ...
        for row in rows:
            if isinstance(row, dict) and str(row.get("symbol") or "") == expected:
                selected[f"{day}|{expected}"] = dict(row)
                break
    return selected


def _ensure_indexes(db: Any) -> None:
    collection = db[TEMPORAL_LEADERSHIP_REGIME_RESEARCH_COLLECTION]
    collection.create_index(
        [("run_id", ASCENDING), ("processing_id", ASCENDING), ("period_start", ASCENDING), ("period_end", ASCENDING), ("created_at", DESCENDING)],
        name="ix_leadership_regime_scope_created",
    )
    collection.create_index([("id", ASCENDING)], unique=True, name="uq_leadership_regime_id")
...
def get_persisted(
    ...
    *,
    ...
    row = db[TEMPORAL_LEADERSHIP_REGIME_RESEARCH_COLLECTION].find_one(
...
def _save(db: Any, document: dict[str, Any]) -> dict[str, Any]:
    _ensure_indexes(db)
    db[TEMPORAL_LEADERSHIP_REGIME_RESEARCH_COLLECTION].insert_one(dict(document))
    return document
...
def build_and_persist(
    ...
    *,
    ...
    selected_by_day = {
        _date_key(row.get("timestamp")): str(row.get("selected_asset") or row.get("strategy_research_control_asset") or "CASH")
...
def delete_run_results(db: Any, run_id: str) -> int:
    return int(db[TEMPORAL_LEADERSHIP_REGIME_RESEARCH_COLLECTION].delete_many({"run_id": str(run_id)}).deleted_count or 0)
