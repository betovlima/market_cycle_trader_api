...
from ..services.counterfactual_rotation_advantage import (
    build_counterfactual_rotation_advantage_policy,
    fit_counterfactual_rotation_advantage,
)


def run_counterfactual_rotation_advantage(
    ...
    if progress_callback is not None:
        progress_callback(
            ...
            (
                ...
                "LightGBM opportunity model + Counterfactual Rotation Advantage"
            ),
            0,
        )
    ...
    for repetition in range(repetitions):
        ...
        for fold_position, fold in enumerate(folds, start=1):
            ...
            report(
                ...
                (
                    f"Run {run_index}/{repetitions} — "
                    f"fold {fold_position}/{total_folds} — "
                    "fitting next-decision counterfactual rotation-advantage model"
                ),
                repetition,
            )
            fitted_model = fit_counterfactual_rotation_advantage(
            ...
            policies[fold_id] = build_counterfactual_rotation_advantage_policy(
        ...
        report(
            ...
            (
                f"Run {run_index}/{repetitions} — "
                "simulating out-of-sample counterfactual rotation-advantage portfolio"
            ),
            repetition,
        )
        ...
        if trade_callback is not None:
            def wrapped_trade_callback(
                ...
                *,
                ...
                payload.update(
                    {
                        "model_family": "lightgbm_counterfactual_rotation_advantage",
                        "random_seed": _seed,
                        "repetition_index": _run,
                        "model": "LightGBM + Counterfactual Rotation Advantage",
                    }
                )
                trade_callback(payload)

        result = _simulate_exact(
            "lightgbm_counterfactual_rotation_advantage",
            scheduled,
            frames,
            symbols,
            all_decision_dates,
            ...
            model_label="LightGBM + Counterfactual Rotation Advantage",
            method_line=(
                "- LightGBM identifies the current Top-1 growth opportunity; "
                "a supervised counterfactual model estimates only the next-"
                "decision net log-return advantage of ROTATE versus HOLD. "
                "CASH is intentionally excluded to isolate rotation quality."
            ),
        )
        result.backend = "lightgbm_counterfactual_rotation_advantage"
        result.metrics.update(
            {
                "backend": result.backend,
                "model_family": "lightgbm_counterfactual_rotation_advantage",
                "strategy_label": "LightGBM + Counterfactual Rotation Advantage",
                "random_seed": seed,
                ...
                "counterfactual_rotation_advantage_folds": fold_models,
