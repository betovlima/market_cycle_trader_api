...
def resolve_qrdqn_compute_plan(config: Any) -> RotationComputePlan:
    requested = str(
        getattr(config, "rotation_accelerator", "auto")
    ).strip().lower()
    allow_fallback = bool(
        getattr(config, "rotation_allow_cpu_fallback", True)
    )
...
def _train_qrdqn(
    ...
    early_stopping_enabled = bool(
        getattr(config, "qrdqn_early_stopping_enabled", False)
    )
    min_training_steps = int(
        getattr(config, "qrdqn_min_training_steps", 5_000)
    )
...
def run_rotation_models(
    ...
    xgb_repetitions = (
        int(getattr(config, "rotation_xgb_repetitions", 1))
        if "xgboost_utility" in selected_models
        else 0
    )
    qrdqn_repetitions = (
        int(getattr(config, "rotation_qrdqn_repetitions", 1))
        if "qrdqn" in selected_models
        else 0
    )
    seed_step = int(getattr(config, "rotation_seed_step", 1_000))
    ...
    if progress_callback is not None:
        progress_callback(
            ...
            (
                ...
                f"parallel_models={bool(getattr(config, 'rotation_parallel_models', True))}"
            ),
            0,
        )
    ...
    def run_qrdqn_family() -> list[RotationRunResult]:
        ...
        requested_workers = max(
            1,
            int(getattr(config, "qrdqn_parallel_folds", 2)),
        )
        ...
        for repetition in range(qrdqn_repetitions):
            ...
            def run_fold(
                ...
                def update_fold(local_fraction: float) -> None:
                    ...
                    report_family(
                        ...
                        (
                            ...
                            f"— {effective_workers} fold worker(s)"
                        ),
                    )
                ...
                normalization = _normalization(frames, train_dates, rep_config)
            ...
            result.metrics.update(
                {
                    ...
                    "qrdqn_parallel_folds_requested": requested_workers,
                    "qrdqn_parallel_folds_effective": effective_workers,
                    "qrdqn_torch_num_threads": torch_runtime_num_threads,
                    "qrdqn_torch_thread_policy": "runtime_default_unchanged",
            ...
            result.summary += f"Parallel fold workers: {effective_workers}\n"
            result.summary += (
                "PyTorch CPU thread policy: runtime default unchanged"
                + (
                    f" ({torch_runtime_num_threads} thread(s))"
                    if torch_runtime_num_threads is not None
                    else ""
                )
                + "\n"
            )
            ...
            report_family(
                "qrdqn",
                (repetition + 1) / qrdqn_repetitions,
