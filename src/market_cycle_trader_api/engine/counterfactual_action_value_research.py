...
from ..services.counterfactual_action_value import (
    build_counterfactual_action_value_policy,
    fit_counterfactual_action_value,
)


def run_counterfactual_action_value(
    ...
    if progress_callback is not None:
        progress_callback(
            ...
            (
                ...
                "LightGBM opportunity model + Counterfactual Action Value"
            ),
            0,
        )
    ...
    for repetition in range(repetitions):
        ...
        for fold_position, fold in enumerate(folds, start=1):
            ...
            report(
                ...
                (
                    f"Run {run_index}/{repetitions} — "
                    f"fold {fold_position}/{total_folds} — "
                    "fitting supervised counterfactual action-value model"
                ),
                repetition,
            )
            fitted_model = fit_counterfactual_action_value(
            ...
            policies[fold_id] = build_counterfactual_action_value_policy(
        ...
        report(
            ...
            (
                f"Run {run_index}/{repetitions} — "
                "simulating out-of-sample counterfactual action-value portfolio"
            ),
            repetition,
        )
        ...
        if trade_callback is not None:
            def wrapped_trade_callback(
                ...
                *,
                ...
                payload.update(
                    {
                        "model_family": "lightgbm_counterfactual_action_value",
                        "random_seed": _seed,
                        "repetition_index": _run,
                        "model": "LightGBM + Counterfactual Action Value",
                    }
                )
                trade_callback(payload)

        result = _simulate_exact(
            "lightgbm_counterfactual_action_value",
            scheduled,
            frames,
            symbols,
            all_decision_dates,
            ...
            model_label="LightGBM + Counterfactual Action Value",
            method_line=(
                "- LightGBM ranks future growth opportunity; a supervised "
                "counterfactual model estimates the Strategy-weighted future "
                "net log return of CASH, HOLD and rotating to the current Top-1."
            ),
        )
        result.backend = "lightgbm_counterfactual_action_value"
        result.metrics.update(
            {
                "backend": result.backend,
                "model_family": "lightgbm_counterfactual_action_value",
                "strategy_label": "LightGBM + Counterfactual Action Value",
                "random_seed": seed,
                ...
                "counterfactual_action_value_folds": fold_models,
