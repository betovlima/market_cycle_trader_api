...
from ..services.counterfactual_rotation_advantage_v4 import (
    build_counterfactual_rotation_advantage_v4_policy,
    fit_counterfactual_rotation_advantage_v4,
)


def run_counterfactual_rotation_advantage_v4(
    ...
    for repetition in range(repetitions):
        ...
        for fold_position, fold in enumerate(folds, start=1):
            ...
            fitted_model = fit_counterfactual_rotation_advantage_v4(
            ...
            policies[fold_id] = build_counterfactual_rotation_advantage_v4_policy(
            ...
            def wrapped_trade_callback(
                ...
                *,
                ...
                payload.update(
                    {
                        "model_family": (
                            "lightgbm_counterfactual_rotation_advantage_v4"
                        ),
                        "random_seed": _seed,
                        "repetition_index": _run,
                        "model": (
                            "LightGBM + Counterfactual Rotation Advantage v4"
                        ),
                    }
                )
        ...
        result = _simulate_exact(
            ...
            model_label="LightGBM + Counterfactual Rotation Advantage v4",
            method_line=(
                "- LightGBM identifies the current Top-1 growth opportunity; "
                "a supervised counterfactual model estimates the relative net "
                "log-return persistence advantage of ROTATE versus HOLD over "
                "the shortest frozen Strategy target horizon. The live policy "
                "is still re-evaluated every session and CASH remains excluded."
            ),
        )
        result.backend = "lightgbm_counterfactual_rotation_advantage_v4"
        result.metrics.update(
            {
                ...
                "counterfactual_rotation_advantage_v4_folds": fold_models,
