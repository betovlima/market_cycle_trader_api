...
from ..services.counterfactual_action_advantage import (
    build_counterfactual_action_advantage_policy,
    fit_counterfactual_action_advantage,
)


def run_counterfactual_action_advantage(
    ...
    for repetition in range(repetitions):
        ...
        for fold_position, fold in enumerate(folds, start=1):
            ...
            fitted_model = fit_counterfactual_action_advantage(
            ...
            policies[fold_id] = build_counterfactual_action_advantage_policy(
        ...
        if trade_callback is not None:
            def wrapped_trade_callback(
                ...
                *,
                ...
                payload.update(
                    {
                        "model_family": "lightgbm_counterfactual_action_advantage",
                        "random_seed": _seed,
                        "repetition_index": _run,
                        "model": "LightGBM + Counterfactual Action Advantage",
                    }
                )
                trade_callback(payload)

        result = _simulate_exact(
            "lightgbm_counterfactual_action_advantage",
            scheduled,
            frames,
            symbols,
            all_decision_dates,
            ...
            model_label="LightGBM + Counterfactual Action Advantage",
            method_line=(
                "- LightGBM ranks future growth opportunity; a supervised "
                "counterfactual model estimates the incremental next-decision "
                "net log-return advantage of changing from the current state "
                "to CASH or the current Top-1."
            ),
        )
        result.backend = "lightgbm_counterfactual_action_advantage"
        result.metrics.update(
            {
                "backend": result.backend,
                "model_family": "lightgbm_counterfactual_action_advantage",
                "strategy_label": "LightGBM + Counterfactual Action Advantage",
                "random_seed": seed,
                ...
                "counterfactual_action_advantage_folds": fold_models,
