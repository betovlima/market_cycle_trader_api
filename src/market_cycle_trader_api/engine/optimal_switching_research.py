...
from ..services.optimal_switching_fqi import (
    build_optimal_switching_policy,
    fit_optimal_switching_fqi,
)


def run_optimal_switching_fqi(
    ...
    if progress_callback is not None:
        progress_callback(
            ...
            (
                ...
                "LightGBM opportunity model + Fitted Q Iteration"
            ),
            0,
        )
    ...
    for repetition in range(repetitions):
        ...
        for fold_position, fold in enumerate(folds, start=1):
            ...
            report(
                fold_base + fold_span * 0.38,
                (
                    f"Run {run_index}/{repetitions} — "
                    f"fold {fold_position}/{total_folds} — "
                    "fitting Fitted Q Iteration switching policy"
                ),
                repetition,
            )
            fitted_q = fit_optimal_switching_fqi(
            ...
            fold_fqi.append({"fold_id": fold_id, **fitted_q.metadata()})
            ...
            final_models = challenger._lightgbm_fit_models(
                ...
                progress_callback=phase_progress(
                    "opportunity final training", 0.52, 0.90
                ),
                technical_log_callback=technical_log_callback,
            )
            policies[fold_id] = build_optimal_switching_policy(
                fitted_q=fitted_q,
        ...
        report(
            ...
            (
                f"Run {run_index}/{repetitions} — "
                "simulating out-of-sample optimal switching portfolio"
            ),
            repetition,
        )
        ...
        if trade_callback is not None:
            def wrapped_trade_callback(
                ...
                *,
                ...
                payload.update(
                    {
                        "model_family": "lightgbm_fqi_optimal_switching",
                        "random_seed": _seed,
                        "repetition_index": _run,
                        "model": "LightGBM + FQI Optimal Switching",
                    }
                )
                trade_callback(payload)

        result = _simulate_exact(
            "lightgbm_fqi_optimal_switching",
            scheduled,
            frames,
            symbols,
            all_decision_dates,
            ...
            model_label="LightGBM + FQI Optimal Switching",
            method_line=(
                "- LightGBM estimates cross-asset opportunity utility; "
                "Fitted Q Iteration learns the sequential value of "
                "HOLD, ROTATE and CASH from pre-test calibration transitions."
            ),
        )
        result.backend = "lightgbm_fqi_optimal_switching"
        result.metrics.update(
            {
                "backend": result.backend,
                "model_family": "lightgbm_fqi_optimal_switching",
                "strategy_label": "LightGBM + FQI Optimal Switching",
                "random_seed": seed,
                ...
                "optimal_switching_fqi_folds": fold_fqi,
