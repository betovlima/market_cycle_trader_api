...
def _shadow_capital_study(
    ...
    *,
    ...
    trend_capture = decision_policy == "trend_capture_hysteresis"
                ...
                if current_symbol is None:
                    ...
                    if should_exit:
                        if (
                            ...
                            target_symbol = None
                            action = "sell"
                            reason = "severe_risk_exit" if severe_risk_exit else "incumbent_persistence_below_exit"
                            cash_recovery = True
                            cash_age = 1
                            position_age = 0
                        ...
                        if (
                            ...
                            and challenger_confirmation > 0.58
                        ):
                            target_symbol = best_symbol
                            action = "rotate"
...
def _multi_horizon_components(
    ...
    *,
    ...
    result["entry_risk_multiplier"] = np.clip(0.46 + 0.54 * np.sqrt(risk_safety), 0.0, 1.0)
    result["risk_adjusted_entry_score"] = np.clip(
        result["opportunity_gate_score"] * result["entry_risk_multiplier"], 0.0, 1.0
    )
    risk_deterioration = np.maximum(0.0, 0.20 - risk_safety)
    result["incumbent_persistence_score"] = np.clip(
        0.42 * result["long_profit_confirmation"]
        + 0.25 * risk_safety
        + 0.12 * result["short_profit_consensus"]
        + 0.11 * result["horizon_agreement"]
        + 0.10 * result["long_trend_support"]
        - 0.24 * risk_deterioration,
        0.0,
        1.0,
    )
...
def _multi_horizon_observation_rows(frame: pd.DataFrame, horizons: list[int]) -> list[dict[str, Any]]:
    ...
    base_columns = [
        ...
        "opportunity_gate_score", "risk_adjusted_entry_score", "entry_risk_multiplier", "entry_threshold",
        "reentry_margin", "reentry_decay_sessions", "hold_score", "incumbent_persistence_score",
...
def run_temporal_intelligence(
    ...
    *,
        ...
        for horizon in horizons:
            ...
            if progress_callback:
                progress_callback(
                    ...
                    f"Temporal Decision Intelligence v5 fold {fold_position}/{len(folds)} · {horizon} sessions",
                )
    ...
    multi_horizon_shadow = _shadow_capital_study(
        ...
        decision_policy="trend_capture_hysteresis",
    ...
    for fold in folds:
        ...
        fold_capital = _shadow_capital_study(
            fold_frame, open_prices, common_dates, config, decision_policy="trend_capture_hysteresis"
    ...
    if progress_callback:
        progress_callback(99.0, "Finalizing Multi-Horizon Temporal Decision Intelligence v5 trend-capture metrics")
    return {
        "experiment": "temporal_decision_intelligence_v5_trend_capture_hysteresis",
        "model_family": "lightgbm_utility",
        "model_label": "LightGBM Multi-Horizon Temporal Decision Intelligence v5 — Trend Capture + Hysteresis",
        ...
        "decision_policy": {
            ...
            "opportunity_gate": "INVEST versus CASH is decided after multi-horizon Top-1 selection; validated risk now multiplicatively reduces entry authority so an exceptional relative opportunity must compensate for weak risk safety",
            "entry": "ENTRY uses the shortest two configured horizons for timing, applies a multi-horizon risk multiplier, and never requires absolute P(profit) to exceed a fixed barrier breakeven probability",
            "hold_exit": "medium/long confirmation protects a healthy incumbent from short-horizon noise; a challenger must clear a dynamic incumbent-aware rotation hurdle, while severe multi-horizon risk can still force an immediate exit with no minimum holding period",
            "cash": "after a deterioration exit, CASH becomes a persistent state through a decaying re-entry margin; exceptional evidence may re-enter immediately, while marginal one-session rebounds remain in CASH until evidence recovers",
            "hysteresis": "ENTRY, HOLD/ROTATE, EXIT, and RE-ENTRY use different evidence thresholds; the policy does not impose a rigid minimum holding period",
