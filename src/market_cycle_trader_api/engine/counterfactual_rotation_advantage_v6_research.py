...
from ..services.counterfactual_rotation_advantage_v6 import (
    build_counterfactual_rotation_advantage_v6_policy,
    fit_counterfactual_rotation_advantage_v6,
)


def run_counterfactual_rotation_advantage_v6(
    ...
    for repetition in range(repetitions):
        ...
        for fold_position, fold in enumerate(folds, start=1):
            ...
            fitted_model = fit_counterfactual_rotation_advantage_v6(
            ...
            policies[fold_id] = build_counterfactual_rotation_advantage_v6_policy(
            ...
            def wrapped_trade_callback(
                ...
                *,
                ...
                payload.update(
                    {
                        "model_family": (
                            "lightgbm_counterfactual_rotation_advantage_v6"
                        ),
                        "random_seed": _seed,
                        "repetition_index": _run,
                        "model": (
                            "LightGBM + Counterfactual Rotation Advantage v6"
                        ),
                    }
                )
        ...
        result = _simulate_exact(
            ...
            model_label="LightGBM + Counterfactual Rotation Advantage v6",
            method_line=(
                "- LightGBM identifies the current Top-1 growth opportunity; "
                "the persistent ROTATE-versus-HOLD target remains unchanged from v4, "
                "but the final decision model is trained only on incumbent/challenger "
                "states visited by one feasible bootstrap policy trajectory. "
                "CASH remains excluded."
            ),
        )
        result.backend = "lightgbm_counterfactual_rotation_advantage_v6"
        result.metrics.update(
            {
                ...
                "counterfactual_rotation_advantage_v6_folds": fold_models,
