...
from ..infrastructure.persistence.mongo_repository import (
    TEMPORAL_FRAGILE_INCUMBENT_RESEARCH_COLLECTION,
...
def _ensure_indexes(db: Any) -> None:
    collection = db[TEMPORAL_FRAGILE_INCUMBENT_RESEARCH_COLLECTION]
    collection.create_index(
        [("run_id", ASCENDING), ("processing_id", ASCENDING), ("period_start", ASCENDING), ("period_end", ASCENDING), ("created_at", DESCENDING)],
        name="ix_fragile_incumbent_scope_created",
    )
    collection.create_index([("id", ASCENDING)], unique=True, name="uq_fragile_incumbent_id")
...
def get_persisted(
    ...
    *,
    ...
    row = db[TEMPORAL_FRAGILE_INCUMBENT_RESEARCH_COLLECTION].find_one(query, {"_id": 0}, sort=[("created_at", DESCENDING)])
    return bson_value(row) if row is not None else None


def _save(db: Any, document: dict[str, Any]) -> dict[str, Any]:
    _ensure_indexes(db)
    db[TEMPORAL_FRAGILE_INCUMBENT_RESEARCH_COLLECTION].insert_one(dict(document))
    return document
...
def delete_run_results(db: Any, run_id: str) -> int:
    return int(db[TEMPORAL_FRAGILE_INCUMBENT_RESEARCH_COLLECTION].delete_many({"run_id": str(run_id)}).deleted_count or 0)
