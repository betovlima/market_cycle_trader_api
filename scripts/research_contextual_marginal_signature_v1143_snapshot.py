...
SCRIPT_VERSION = "contextual-marginal-signature-v1.0.14.3"
...
def _runtime_mongo_settings(args: argparse.Namespace, mongo_repository: Any) -> tuple[str, str]:
    ...
    if not mongo_uri:
        raise RuntimeError(
            "MongoDB connection is not configured after loading the project environment. "
            "Set MONGO_URL/MONGO_URI in .env or pass --mongo-uri explicitly."
        )
    if not database_name:
        raise RuntimeError(
            "MongoDB database is not configured after loading the project environment. "
            "Set MONGO_DATABASE in .env or pass --database explicitly."
        )

    # mongo_repository historically cached these values at import time. Research
    # scripts load .env at runtime, so synchronize the module globals explicitly
    # before market_data.load_market_bars() opens any secondary Mongo clients.
