...
import research_asset_rotation_leadership_v12 as v12  # noqa: E402

SCRIPT_VERSION = "asset-rotation-leadership-v1.3"
...
def _placeholder_prediction_rows(
    ...
    # These rows exist only so resume knows every fold was already evaluated.
    # v1.3 removes non-finite predictions before cross-sectional ranking, so
    # they cannot change another asset's rank or leadership metrics.
...
def main() -> int:
    ...
    # v1.2 provides the agreed rotation-only final gate and detailed progress logs.
    v12.SCRIPT_VERSION = SCRIPT_VERSION
    return int(v12.main())
