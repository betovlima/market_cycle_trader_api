...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Select external assets by positive OOS marginal rotation contribution "
            "without a full backtest per candidate."
        )
    )
...
def _candidate_pool(
    ...
    if "leadership_qualified" in frame.columns:
        ...
        raise RuntimeError(
            "asset_rotation_qualification.csv needs leadership_qualified or qualified."
        )

    return sorted(
        {
            symbol
            for symbol in frame.loc[accepted, "symbol"].tolist()
            if symbol and symbol not in baseline
        }
    )
    ...
    frozen.update(
        {
            ...
            "selection_rule": (
                ...
                if control
                    ...
                    "immutable Strategy baseline plus greedy positive OOS marginal "
                    "rotation contribution among leadership-qualified candidates"
                )
            ),
            ...
            "marginal_rejected_candidates": (
                [] if control else [s for s in pool if s not in set(selected)]
            ),
...
def main() -> int:
    ...
    baseline = list(
        dict.fromkeys(
            str(x).strip().upper()
            for x in source.get("original_assets") or []
            if str(x).strip()
        )
    )
    if len(baseline) < 2:
