...
from market_cycle_trader_api.engine.counterfactual_rotation_advantage_v6_research import (  # noqa: E402
    run_counterfactual_rotation_advantage_v6,
)
from market_cycle_trader_api.services.counterfactual_rotation_advantage_v6 import (  # noqa: E402
    CounterfactualRotationAdvantageV6Settings,
)
...
SCRIPT_VERSION = "counterfactual-rotation-advantage-v6.0.0"
CRA6_SETTINGS = CounterfactualRotationAdvantageV6Settings(enabled=True)
...
def _default_output_dir(pre: argparse.Namespace) -> Path:
    return (
        ...
        / "research_output"
        / (
            f"counterfactual_rotation_advantage_v6_strategy_{pre.strategy_sequence}_"
...
def _patch_snapshot() -> None:
    ...
    def snapshot(strategy: dict[str, Any]):
        ...
        merged["counterfactual_rotation_advantage_v6"] = CRA6_SETTINGS.as_dict()
...
def _research_source_sha256() -> str:
    digest = hashlib.sha256()
    relative_paths = [
        "scripts/research_counterfactual_rotation_advantage_v6.py",
        "src/market_cycle_trader_api/engine/counterfactual_rotation_advantage_v6_research.py",
        "src/market_cycle_trader_api/services/counterfactual_rotation_advantage_v6.py",
...
def _decision_statistics(predictions_path: Path) -> dict[str, Any]:
    ...
    if "decision_reason" in frame.columns:
        ...
        result.update(
            {
                "cra6_enter_decisions": int(counts.get("CRA6_ENTER_TOP1", 0)),
                "cra6_rotate_decisions": int(
                    counts.get("CRA6_ROTATE_POSITIVE_PERSISTENT_ADVANTAGE", 0)
                ),
                "cra6_hold_current_top1_decisions": int(
                    counts.get("CRA6_HOLD_CURRENT_TOP1", 0)
                ),
                "cra6_hold_nonpositive_advantage_decisions": int(
                    counts.get("CRA6_HOLD_NONPOSITIVE_PERSISTENT_ADVANTAGE", 0)
                ),
            }
        )
    ...
    if "cra6_rotation_advantage" in frame.columns:
        values = pd.to_numeric(frame["cra6_rotation_advantage"], errors="coerce").dropna()
        result["mean_cra6_rotation_advantage"] = (
            float(values.mean()) if not values.empty else None
        )
        result["median_cra6_rotation_advantage"] = (
            float(values.median()) if not values.empty else None
        )
        result["positive_cra6_rotation_advantage_days"] = int((values > 0.0).sum())
        result["negative_cra6_rotation_advantage_days"] = int((values < 0.0).sum())
    for column in ("cra6_training_decision_dates", "cra6_seed_decision_dates"):
        if column in frame.columns:
            values = pd.to_numeric(frame[column], errors="coerce").dropna()
            result[f"maximum_{column}"] = (
        ...
        rows.append(
            {
                ...
                "cra6_ending_capital": variant_capital,
...
def _finalize(
    *,
    ...
    if not manifest_path.exists() or not summary_path.exists():
        raise RuntimeError(
            "Counterfactual Rotation Advantage v6 output is incomplete; expected "
    ...
    manifest.update(
        {
            "experiment": "counterfactual_rotation_advantage_v6",
            "script_version": SCRIPT_VERSION,
            "baseline_artifact_source": str(baseline_source),
            "frozen_universe_reused": True,
            "counterfactual_rotation_advantage_v6": CRA6_SETTINGS.as_dict(),
    ...
    universe_scale._write_csv(
        output_dir / "counterfactual_rotation_advantage_v6_comparison.csv",
        comparison,
    )
    universe_scale._write_json(
        output_dir / "counterfactual_rotation_advantage_v6_comparison.json",
        {
            "schema_version": 1,
            "script_version": SCRIPT_VERSION,
            "algorithm": "Policy-Reachable Counterfactual Rotation Advantage",
            "hypothesis": (
                "The persistent ROTATE-versus-HOLD target should generalize better "
                "when the final model is trained only on states visited by one feasible "
                "calibration policy trajectory rather than every historically reachable "
                "Top-1 incumbent."
            ),
            "settings": CRA6_SETTINGS.as_dict(),
            "series": comparison.replace({np.nan: None}).to_dict(orient="records"),
        },
    )

    archive_path = universe_scale._archive_output_dir(output_dir)
    print("\n=== COUNTERFACTUAL ROTATION ADVANTAGE V6 COMPARISON ===", flush=True)
    for row in comparison.to_dict(orient="records"):
        ...
        universe_scale._log(
            ...
            f"CRA6=${float(row['cra6_ending_capital']):,.2f}; "
            f"delta={delta_text}; "
            f"rotations={int(row.get('cra6_rotations') or 0)}; "
            f"cash_days={int(row.get('cra6_cash_days') or 0)}."
        )
    universe_scale._log(f"Counterfactual Rotation Advantage v6 artifacts: {output_dir}")
    universe_scale._log(f"Counterfactual Rotation Advantage v6 archive: {archive_path}")
    return archive_path
...
def main() -> int:
    ...
    try:
        if not pre.finalize_only:
            ...
            universe_scale.run_rotation_models = run_counterfactual_rotation_advantage_v6
