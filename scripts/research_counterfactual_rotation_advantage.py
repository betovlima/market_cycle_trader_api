...
from market_cycle_trader_api.engine.counterfactual_rotation_advantage_research import (  # noqa: E402
    run_counterfactual_rotation_advantage,
)
from market_cycle_trader_api.services.counterfactual_rotation_advantage import (  # noqa: E402
    CounterfactualRotationAdvantageSettings,
)
...
SCRIPT_VERSION = "counterfactual-rotation-advantage-v3.0.0"
CRA_SETTINGS = CounterfactualRotationAdvantageSettings(enabled=True)
...
def _default_output_dir(pre: argparse.Namespace) -> Path:
    return (
        ...
        / "research_output"
        / (
            f"counterfactual_rotation_advantage_strategy_{pre.strategy_sequence}_"
...
def _patch_snapshot() -> None:
    ...
    def snapshot(strategy: dict[str, Any]):
        ...
        merged["counterfactual_rotation_advantage"] = CRA_SETTINGS.as_dict()
...
def _research_source_sha256() -> str:
    digest = hashlib.sha256()
    relative_paths = [
        "scripts/research_counterfactual_rotation_advantage.py",
        "src/market_cycle_trader_api/engine/counterfactual_rotation_advantage_research.py",
        "src/market_cycle_trader_api/services/counterfactual_rotation_advantage.py",
...
def _decision_statistics(predictions_path: Path) -> dict[str, Any]:
    ...
    if "decision_reason" in frame.columns:
        ...
        result.update(
            {
                "cra_enter_decisions": int(counts.get("CRA_ENTER_TOP1", 0)),
                "cra_rotate_decisions": int(
                    counts.get("CRA_ROTATE_POSITIVE_ADVANTAGE", 0)
                ),
                "cra_hold_current_top1_decisions": int(
                    counts.get("CRA_HOLD_CURRENT_TOP1", 0)
                ),
                "cra_hold_nonpositive_advantage_decisions": int(
                    counts.get("CRA_HOLD_NONPOSITIVE_ADVANTAGE", 0)
                ),
            }
        )
    ...
    if "cra_rotation_advantage" in frame.columns:
        values = pd.to_numeric(frame["cra_rotation_advantage"], errors="coerce").dropna()
        result["mean_cra_rotation_advantage"] = (
            float(values.mean()) if not values.empty else None
        )
        result["median_cra_rotation_advantage"] = (
            float(values.median()) if not values.empty else None
        )
        result["positive_cra_rotation_advantage_days"] = int((values > 0.0).sum())
        result["negative_cra_rotation_advantage_days"] = int((values < 0.0).sum())
    return result
        ...
        rows.append(
            {
                ...
                "cra_ending_capital": variant_capital,
                ...
                "cra_cagr": _metric(current, "strategy_cagr", "cagr"),
                "baseline_sharpe": _metric(control, "strategy_sharpe", "sharpe"),
                "cra_sharpe": _metric(current, "strategy_sharpe", "sharpe"),
                "baseline_maximum_drawdown": baseline_maxdd,
                "cra_maximum_drawdown": variant_maxdd,
                "baseline_rotations": _metric(
                    control, "capital_rotations", "rotation_count"
                ),
                "cra_rotations": _metric(current, "capital_rotations", "rotation_count"),
                "baseline_cash_days": _metric(control, "cash_days"),
                "cra_cash_days": _metric(current, "cash_days"),
...
def _finalize(
    *,
    ...
    if not manifest_path.exists() or not summary_path.exists():
        raise RuntimeError(
            "Counterfactual Rotation Advantage output is incomplete; expected "
    ...
    manifest.update(
        {
            "experiment": "counterfactual_rotation_advantage",
            "script_version": SCRIPT_VERSION,
            "baseline_artifact_source": str(baseline_source),
            "frozen_universe_reused": True,
            "counterfactual_rotation_advantage": CRA_SETTINGS.as_dict(),
            ...
            "rotation_advantage_label_horizon_sessions": 1,
    ...
    universe_scale._write_csv(
        output_dir / "counterfactual_rotation_advantage_comparison.csv",
        comparison,
    )
    universe_scale._write_json(
        output_dir / "counterfactual_rotation_advantage_comparison.json",
        {
            "schema_version": 1,
            "script_version": SCRIPT_VERSION,
            "algorithm": "Supervised Counterfactual Rotation Advantage",
            "hypothesis": (
                "A learned incremental ROTATE-versus-HOLD advantage can reduce "
                "unnecessary Top-1 chasing while preserving full market exposure, "
                "without hand-written switch or minimum-hold thresholds."
            ),
            "settings": CRA_SETTINGS.as_dict(),
            "series": comparison.replace({np.nan: None}).to_dict(orient="records"),
        },
    )

    archive_path = universe_scale._archive_output_dir(output_dir)
    print("\n=== COUNTERFACTUAL ROTATION ADVANTAGE COMPARISON ===", flush=True)
    for row in comparison.to_dict(orient="records"):
        ...
        universe_scale._log(
            ...
            f"CRA=${float(row['cra_ending_capital']):,.2f}; "
            f"delta={delta_text}; "
            f"rotations={int(row.get('cra_rotations') or 0)}; "
            f"cash_days={int(row.get('cra_cash_days') or 0)}."
        )
    universe_scale._log(f"Counterfactual Rotation Advantage artifacts: {output_dir}")
    universe_scale._log(f"Counterfactual Rotation Advantage archive: {archive_path}")
    return archive_path
...
def main() -> int:
    ...
    try:
        if not pre.finalize_only:
            ...
            universe_scale.run_rotation_models = run_counterfactual_rotation_advantage
