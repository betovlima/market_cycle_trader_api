...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Run the complete research flow: revalidate all existing/candidate assets by point-in-time "
            "timing vs same-asset buy-and-hold, freeze the universe, then run exactly one full Strategy Backtest."
        )
    )
...
def main() -> int:
    ...
    _append(qualification, "--output-dir", args.output_dir)
    ...
    _append(final_backtest, "--output-dir", args.output_dir)
    _append(final_backtest, "--baseline-job-id", args.baseline_job_id)

    print("=== PHASE 2: ONE FULL STRATEGY BACKTEST OF FROZEN QUALIFIED UNIVERSE ===", flush=True)
