...
from market_cycle_trader_api.engine.counterfactual_rotation_advantage_v7_research import (  # noqa: E402
    run_counterfactual_rotation_advantage_v7,
)
from market_cycle_trader_api.services.counterfactual_rotation_advantage_v7 import (  # noqa: E402
    CounterfactualRotationAdvantageV7Settings,
)
...
SCRIPT_VERSION = "counterfactual-rotation-advantage-v7.0.0"
CRA7_SETTINGS = CounterfactualRotationAdvantageV7Settings(enabled=True)
...
def _default_output_dir(pre: argparse.Namespace) -> Path:
    return (
        ...
        / "research_output"
        / (
            f"counterfactual_rotation_advantage_v7_strategy_{pre.strategy_sequence}_"
...
def _patch_snapshot() -> None:
    ...
    def snapshot(strategy: dict[str, Any]):
        ...
        merged["counterfactual_rotation_advantage_v7"] = CRA7_SETTINGS.as_dict()
...
def _research_source_sha256() -> str:
    digest = hashlib.sha256()
    relative_paths = [
        "scripts/research_counterfactual_rotation_advantage_v7.py",
        "src/market_cycle_trader_api/engine/counterfactual_rotation_advantage_v7_research.py",
        "src/market_cycle_trader_api/services/counterfactual_rotation_advantage_v7.py",
...
def _decision_statistics(predictions_path: Path) -> dict[str, Any]:
    ...
    if "decision_reason" in frame.columns:
        ...
        result.update(
            {
                "cra7_enter_decisions": int(counts.get("CRA7_ENTER_TOP1", 0)),
                "cra7_rotate_decisions": int(
                    counts.get("CRA7_ROTATE_POSITIVE_POSTERIOR_ADVANTAGE", 0)
                ),
                "cra7_hold_current_top1_decisions": int(
                    counts.get("CRA7_HOLD_CURRENT_TOP1", 0)
                ),
                "cra7_hold_nonpositive_advantage_decisions": int(
                    counts.get("CRA7_HOLD_NONPOSITIVE_POSTERIOR_ADVANTAGE", 0)
                ),
            }
        )
    ...
    if "cra7_rotation_advantage" in frame.columns:
        values = pd.to_numeric(frame["cra7_rotation_advantage"], errors="coerce").dropna()
        result["mean_cra7_rotation_advantage"] = (
            float(values.mean()) if not values.empty else None
        )
        result["median_cra7_rotation_advantage"] = (
            float(values.median()) if not values.empty else None
        )
        result["positive_cra7_rotation_advantage_days"] = int((values > 0.0).sum())
        result["negative_cra7_rotation_advantage_days"] = int((values < 0.0).sum())
    for column in (
        "cra7_training_decision_dates",
        "cra7_seed_decision_dates",
        "cra7_sample_feature_ratio",
        ...
        rows.append(
            {
                ...
                "cra7_ending_capital": variant_capital,
...
def _finalize(
    *,
    ...
    if not manifest_path.exists() or not summary_path.exists():
        raise RuntimeError(
            "Counterfactual Rotation Advantage v7 output is incomplete; expected "
    ...
    universe_scale._write_csv(
        output_dir / "counterfactual_rotation_advantage_v7_comparison.csv",
        comparison,
    )
    universe_scale._write_json(
        output_dir / "counterfactual_rotation_advantage_v7_comparison.json",
        {
            "schema_version": 1,
            "script_version": SCRIPT_VERSION,
            "algorithm": (
                "Policy-Reachable Bayesian Counterfactual Rotation Advantage"
            ),
            "hypothesis": (
                "The v6 policy-reachable state hypothesis should be evaluated with "
                "an estimator whose regularization remains identifiable when the "
                "reachable sample count is smaller than or close to the feature count."
            ),
            "settings": CRA7_SETTINGS.as_dict(),
            "series": comparison.replace({np.nan: None}).to_dict(orient="records"),
        },
    )

    archive_path = universe_scale._archive_output_dir(output_dir)
    print("\n=== COUNTERFACTUAL ROTATION ADVANTAGE V7 COMPARISON ===", flush=True)
    for row in comparison.to_dict(orient="records"):
        ...
        universe_scale._log(
            ...
            f"CRA7=${float(row['cra7_ending_capital']):,.2f}; "
            f"delta={delta_text}; "
            f"rotations={int(row.get('cra7_rotations') or 0)}; "
            f"cash_days={int(row.get('cra7_cash_days') or 0)}."
        )
    universe_scale._log(f"Counterfactual Rotation Advantage v7 artifacts: {output_dir}")
    universe_scale._log(f"Counterfactual Rotation Advantage v7 archive: {archive_path}")
    return archive_path
...
def main() -> int:
    ...
    try:
        if not pre.finalize_only:
            ...
            universe_scale.run_rotation_models = run_counterfactual_rotation_advantage_v7
