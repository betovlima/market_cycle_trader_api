...
from market_cycle_trader_api.engine.counterfactual_rotation_advantage_v5_research import (  # noqa: E402
    run_counterfactual_rotation_advantage_v5,
)
from market_cycle_trader_api.services.counterfactual_rotation_advantage_v5 import (  # noqa: E402
    CounterfactualRotationAdvantageV5Settings,
)
...
SCRIPT_VERSION = "counterfactual-rotation-advantage-v5.0.0"
CRA5_SETTINGS = CounterfactualRotationAdvantageV5Settings(enabled=True)
...
def _default_output_dir(pre: argparse.Namespace) -> Path:
    return (
        ...
        / "research_output"
        / (
            f"counterfactual_rotation_advantage_v5_strategy_{pre.strategy_sequence}_"
...
def _patch_snapshot() -> None:
    ...
    def snapshot(strategy: dict[str, Any]):
        ...
        merged["counterfactual_rotation_advantage_v5"] = CRA5_SETTINGS.as_dict()
...
def _research_source_sha256() -> str:
    digest = hashlib.sha256()
    relative_paths = [
        "scripts/research_counterfactual_rotation_advantage_v5.py",
        "src/market_cycle_trader_api/engine/counterfactual_rotation_advantage_v5_research.py",
        "src/market_cycle_trader_api/services/counterfactual_rotation_advantage_v5.py",
...
def _decision_statistics(predictions_path: Path) -> dict[str, Any]:
    ...
    if "decision_reason" in frame.columns:
        ...
        result.update(
            {
                "cra5_enter_decisions": int(counts.get("CRA5_ENTER_TOP1", 0)),
                "cra5_rotate_decisions": int(
                    counts.get("CRA5_ROTATE_POSITIVE_PERSISTENT_ADVANTAGE", 0)
                ),
                "cra5_hold_current_top1_decisions": int(
                    counts.get("CRA5_HOLD_CURRENT_TOP1", 0)
                ),
                "cra5_hold_nonpositive_advantage_decisions": int(
                    counts.get("CRA5_HOLD_NONPOSITIVE_PERSISTENT_ADVANTAGE", 0)
                ),
            }
        )
    if {"current_asset", "best_asset", "final_action_asset"}.issubset(frame.columns):
        valid = frame[["current_asset", "best_asset", "final_action_asset"]].dropna()
    ...
    if "cra5_rotation_advantage" in frame.columns:
        values = pd.to_numeric(frame["cra5_rotation_advantage"], errors="coerce").dropna()
        result["mean_cra5_rotation_advantage"] = (
            float(values.mean()) if not values.empty else None
        )
        result["median_cra5_rotation_advantage"] = (
            float(values.median()) if not values.empty else None
        )
        result["positive_cra5_rotation_advantage_days"] = int((values > 0.0).sum())
        result["negative_cra5_rotation_advantage_days"] = int((values < 0.0).sum())
    if "cra5_training_memory_blocks" in frame.columns:
        values = pd.to_numeric(frame["cra5_training_memory_blocks"], errors="coerce").dropna()
        result["maximum_training_memory_blocks"] = int(values.max()) if not values.empty else None
    if "cra5_training_memory_decision_dates" in frame.columns:
        values = pd.to_numeric(
            frame["cra5_training_memory_decision_dates"], errors="coerce"
        ).dropna()
        result["maximum_training_memory_decision_dates"] = (
            int(values.max()) if not values.empty else None
        )
    return result
...
def _comparison(baseline_dir: Path, output_dir: Path) -> pd.DataFrame:
    baseline = json.loads(
        (baseline_dir / "universe_scale_summary.json").read_text(encoding="utf-8")
    )
    variant = json.loads(
        (output_dir / "universe_scale_summary.json").read_text(encoding="utf-8")
    )
    baseline_by_size = {
    ...
    for current in list(variant.get("series") or []):
        ...
        baseline_capital = float(
            _metric(control, "strategy_ending_capital", "ending_capital") or 0.0
        )
        variant_capital = float(
            _metric(current, "strategy_ending_capital", "ending_capital") or 0.0
        )
        ...
        rows.append(
            {
                ...
                "cra5_ending_capital": variant_capital,
                ...
                "cra5_cagr": _metric(current, "strategy_cagr", "cagr"),
                "baseline_sharpe": _metric(control, "strategy_sharpe", "sharpe"),
                "cra5_sharpe": _metric(current, "strategy_sharpe", "sharpe"),
                "baseline_maximum_drawdown": baseline_maxdd,
                "cra5_maximum_drawdown": variant_maxdd,
                "baseline_rotations": _metric(
                    control, "capital_rotations", "rotation_count"
                ),
                "cra5_rotations": _metric(current, "capital_rotations", "rotation_count"),
                "baseline_cash_days": _metric(control, "cash_days"),
                "cra5_cash_days": _metric(current, "cash_days"),
    ...
    if not rows:
        raise RuntimeError("No matching baseline universe sizes were found for comparison.")
...
def _finalize(
    *,
    ...
    if not manifest_path.exists() or not summary_path.exists():
        raise RuntimeError(
            "Counterfactual Rotation Advantage v5 output is incomplete; expected "
    ...
    manifest.update(
        {
            "experiment": "counterfactual_rotation_advantage_v5",
            "script_version": SCRIPT_VERSION,
            "baseline_artifact_source": str(baseline_source),
            "frozen_universe_reused": True,
            "counterfactual_rotation_advantage_v5": CRA5_SETTINGS.as_dict(),
            ...
            "test_labels_replayed_into_later_folds": False,
            "rotation_advantage_label_tail_purged": True,
            "rotation_advantage_label_horizon_source": "min(rotation_target_horizons)",
    ...
    universe_scale._write_csv(
        output_dir / "counterfactual_rotation_advantage_v5_comparison.csv",
        comparison,
    )
    universe_scale._write_json(
        output_dir / "counterfactual_rotation_advantage_v5_comparison.json",
        {
            "schema_version": 1,
            "script_version": SCRIPT_VERSION,
            "algorithm": "Supervised Counterfactual Rotation Advantage v5",
            "hypothesis": (
                "Expanding the rotation model's training memory across prior OOS "
                "calibration blocks reduces short-window regime imprinting while "
                "preserving the v4 persistence target, full exposure, and a purely "
                "economic ROTATE-versus-HOLD decision."
            ),
            "settings": CRA5_SETTINGS.as_dict(),
            "series": comparison.replace({np.nan: None}).to_dict(orient="records"),
        },
    )

    archive_path = universe_scale._archive_output_dir(output_dir)
    print("\n=== COUNTERFACTUAL ROTATION ADVANTAGE v5 COMPARISON ===", flush=True)
    for row in comparison.to_dict(orient="records"):
        ...
        universe_scale._log(
            ...
            f"CRA5=${float(row['cra5_ending_capital']):,.2f}; "
            f"delta={delta_text}; "
            f"rotations={int(row.get('cra5_rotations') or 0)}; "
            f"cash_days={int(row.get('cra5_cash_days') or 0)}."
        )
    universe_scale._log(f"Counterfactual Rotation Advantage v5 artifacts: {output_dir}")
    universe_scale._log(f"Counterfactual Rotation Advantage v5 archive: {archive_path}")
    return archive_path
...
def main() -> int:
    ...
    baseline_dir, baseline_temp, baseline_source = resolve_universe_scale_baseline(
    ...
    try:
        if not pre.finalize_only:
            _patch_snapshot()
            universe_scale.CandidateUniverseLoader = FrozenUniverseScaleCandidateLoader
            universe_scale.run_rotation_models = run_counterfactual_rotation_advantage_v5
