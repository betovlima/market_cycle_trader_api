...
from market_cycle_trader_api.engine.counterfactual_action_value_research import (  # noqa: E402
    run_counterfactual_action_value,
)
from market_cycle_trader_api.services.counterfactual_action_value import (  # noqa: E402
    CounterfactualActionValueSettings,
)
...
SCRIPT_VERSION = "counterfactual-action-value-v1.0.0"
CAV_SETTINGS = CounterfactualActionValueSettings(enabled=True)
...
def _default_output_dir(pre: argparse.Namespace) -> Path:
    return (
        ...
        / "research_output"
        / (
            f"counterfactual_action_value_strategy_{pre.strategy_sequence}_"
...
def _patch_snapshot() -> None:
    ...
    def snapshot(strategy: dict[str, Any]):
        ...
        merged["counterfactual_action_value"] = CAV_SETTINGS.as_dict()
...
def _research_source_sha256() -> str:
    digest = hashlib.sha256()
    relative_paths = [
        "scripts/research_counterfactual_action_value.py",
        "src/market_cycle_trader_api/engine/counterfactual_action_value_research.py",
        "src/market_cycle_trader_api/services/counterfactual_action_value.py",
...
def _decision_statistics(predictions_path: Path) -> dict[str, Any]:
    ...
    if "decision_reason" in frame.columns:
        ...
        result.update(
            {
                "cav_hold_decisions": int(counts.get("CAV_HOLD", 0)),
                "cav_rotate_decisions": int(counts.get("CAV_ROTATE", 0)),
                "cav_enter_decisions": int(counts.get("CAV_ENTER", 0)),
                "cav_cash_decisions": int(counts.get("CAV_CASH", 0)),
            }
        )
        ...
        rows.append(
            {
                ...
                "cav_ending_capital": variant_capital,
...
def _finalize(
    *,
    ...
    if not manifest_path.exists() or not summary_path.exists():
        raise RuntimeError(
            "Counterfactual Action Value output is incomplete; expected "
    ...
    universe_scale._write_csv(
        output_dir / "counterfactual_action_value_comparison.csv",
        comparison,
    )
    universe_scale._write_json(
        output_dir / "counterfactual_action_value_comparison.json",
        {
            "schema_version": 1,
            "script_version": SCRIPT_VERSION,
            "algorithm": "Supervised Counterfactual Action Value",
            "hypothesis": (
                "A learned action-value model can decide between CASH, HOLD and "
                "rotating to the current LightGBM Top-1 without hand-written "
                "switch, cash or minimum-hold thresholds."
            ),
            "settings": CAV_SETTINGS.as_dict(),
            "series": comparison.replace({np.nan: None}).to_dict(orient="records"),
        },
    )

    archive_path = universe_scale._archive_output_dir(output_dir)
    print("\n=== COUNTERFACTUAL ACTION VALUE COMPARISON ===", flush=True)
    for row in comparison.to_dict(orient="records"):
        ...
        universe_scale._log(
            ...
            f"CAV=${float(row['cav_ending_capital']):,.2f}; "
            f"delta={delta_text}; "
            f"rotations={int(row.get('cav_rotations') or 0)}; "
            f"cash_days={int(row.get('cav_cash_days') or 0)}."
        )
    universe_scale._log(f"Counterfactual Action Value artifacts: {output_dir}")
    universe_scale._log(f"Counterfactual Action Value archive: {archive_path}")
    return archive_path
